        ...
        const obtenerPokes = async () => {
          ...
          const loop = () =>{
            return new Promise((resolve, reject) => {
                setTimeout(()=>{
                    urlDetalles.forEach(async (url) => {
                        console.log("CONTADORFOREACHURL")
                        console.log("URL"+url)
                        const data = await fetch(`${url}`)
                        const result = await data.json()
                        xxx.push(result.height)
                        setDetallesPokes([ //Error está guardando solo una vez, el log de arriba corre las 10 veces pero este set por algún motivo se está ejecutando 1 sola vez, guardando un solo dato en lugar de acumular todos
                        ...detallesPokes, //está trayendo [objet,Objetct]
                        result.height
                        ]);
                        console.log(detallesPokes);
                        console.log(xxx);
                        console.log("CONTADORFOREACHURL2")//Este consolo log se está ejecutando también una sola vez a diferencia del de arriba que si corre 10
                      });
                }, 500);
